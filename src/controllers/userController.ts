...
class UserController {
    async changeUserProfile(data: Record<(typeof PROFILE_INFO_INPUTS)[number], string>) {
        try {
            ...
            if (error instanceof Error) {
                store.set('profileForm.error', error.message);
            } else {
                store.set('profileForm.error', 'Что-то пошло не так');
            }
        } finally {
    ...
    async changeUserPassword(data: Record<(typeof PASSWORD_INPUTS)[number], string>) {
        try {
            ...
            if (error instanceof Error) {
                store.set('passwordForm.error', error.message);
            } else {
                store.set('passwordForm.error', 'Что-то пошло не так');
            }
        } finally {
