...
class ChatController {
    ...
    async getChats() {
        try {
            ...
            if (error instanceof Error) {
                ...
                store.set('chats.error', 'Что-то пошло не так');
            }
        } finally {
    ...
    async createChat(data: { title: string }) {
        try {
            ...
            if (error instanceof Error) {
                store.set('newChatForm.error', error.message);
            } else {
                store.set('newChatForm.error', 'Что-то пошло не так');
            }
        } finally {
    ...
    async changeChatAvatar(data: FormData) {
        try {
            ...
            if (error instanceof Error) {
                store.set('avatarForm.error', error.message);
            } else {
                store.set('avatarForm.error', 'Что-то пошло не так');
            }
        } finally {
    ...
    async addUserToChat(data: { login: string; chatId: number }) {
        try {
            ...
            if (error instanceof Error) {
                store.set('addUserForm.error', error.message);
            } else {
                store.set('addUserForm.error', 'Что-то пошло не так');
            }
        } finally {
    ...
    async removeUserFromChat(data: { login: string; chatId: number }) {
        try {
            ...
            if (error instanceof Error) {
                store.set('removeUserForm.error', error.message);
            } else {
                store.set('removeUserForm.error', 'Что-то пошло не так');
            }
        } finally {
